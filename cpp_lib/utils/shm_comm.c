...
static int I_LEVEL( const char * level) {
    if (0 == strcmp(level, SHM_LOG_DEBUG))
        return 1;
    if (0 == strcmp(level, SHM_LOG_INFO))
        return 2;
    if (0 == strcmp(level, SHM_LOG_WARN))
        return 3;
...
int	init_memqueue(int mqSize, chn_comm_ctlinfo * pCCInfo, int streamDirect) {
	...
	if (MQ_MIN_SIZE > mqSize) {
		SHM_COMM_LOG(SHM_LOG_ERROR, "mqsize is too small, minsize:%d", MQ_MIN_SIZE);
		return SHM_OPT_FAIL;
	}

	if (0 == is_power_of_2(mqSize)) {
		SHM_COMM_LOG(SHM_LOG_ERROR, "mqSize must be power of 2");
		return SHM_OPT_FAIL;
	}

	if (NULL == pCCInfo) {
		SHM_COMM_LOG(SHM_LOG_ERROR, "pCCInfo is null");
		return SHM_OPT_FAIL;
	}

	if (streamDirect != STREAM_IN_DIRECT && streamDirect != STREAM_OUT_DIRECT) {
		SHM_COMM_LOG(SHM_LOG_ERROR, "streamDirect invalid");
		return SHM_OPT_FAIL;
	}

	unsigned char *head_maped =  (unsigned char *)malloc(mqSize);
	if (NULL == head_maped) {
		SHM_COMM_LOG(SHM_LOG_ERROR, "malloc %d bytes to head_maped fail!", mqSize);
		return SHM_OPT_FAIL;
	}
	
	setCCInfo(head_maped, pCCInfo, streamDirect, mqSize);
    SHM_COMM_LOG(SHM_LOG_INFO, "init_memqueue succ");
...
int init_shmfile(const char * pShmName, int mqSize, chn_comm_ctlinfo * pCCInfo, int streamDirect) {
	...
	if (NULL == pShmName) {
		SHM_COMM_LOG(SHM_LOG_ERROR, "shmfile name is null");
		return SHM_OPT_FAIL;
	}
	...
	if ((fd = open(pShmName, O_RDWR | O_CREAT | O_EXCL, 0777)) == -1) {
		if (errno != EEXIST) {
			SHM_COMM_LOG(SHM_LOG_ERROR, "open fail: %d", errno);
			return SHM_OPT_FAIL;
		} else {
			SHM_COMM_LOG(SHM_LOG_INFO, "file %s exist!", pShmName);
			int ret = remove(pShmName);
			if (0 != ret) {
				SHM_COMM_LOG(SHM_LOG_ERROR, "remove %s fail, errno:%d",
						pShmName, errno);
				return SHM_OPT_FAIL;
			} else {
				fd = open(pShmName, O_RDWR | O_CREAT | O_EXCL, 0777);
				if (-1 == fd) {
					SHM_COMM_LOG(SHM_LOG_ERROR, "open fail again!: %d ", errno);
					return SHM_OPT_FAIL;
				}
			}
		}
	...
	if (ftruncate(fd, mqSize) == -1) {
		SHM_COMM_LOG(SHM_LOG_ERROR, "error truncate the file: %d", errno);
		close(fd);
		return SHM_OPT_FAIL;
	}
	...
	if (head_maped == MAP_FAILED || head_maped == NULL) {
		SHM_COMM_LOG(SHM_LOG_ERROR, "error mmap SHM_FILE: %d", errno);
		perror("mmap");
		close(fd);
		return SHM_OPT_FAIL;
	} else {
		SHM_COMM_LOG(SHM_LOG_INFO, "mmap succ!");
	}
...
int  shm_write(chn_comm_ctlinfo * pCCInfo, void * pChr, uint32_t binSize, int streamDirect) {
	...
	if (NULL == pCCInfo || NULL == pChr || (0 > streamDirect || streamDirect >= MAX_CHN_NUM)) {
		SHM_COMM_LOG(SHM_LOG_ERROR, "NULL == pCCInfo || NULL == pChr, or streamDirect [%d] is illegal", streamDirect);
		return SHM_OPT_FAIL;
	}
	...
	if (SHM_OPT_FAIL == checkMQHead(&(pCCInfo->chn_list[streamDirect]))) {
		...
		while (wLen != binSize)
		{
			if (nLoop < 0)
			{
				SHM_COMM_LOG(SHM_LOG_ERROR, "try out, wLen != binSize");
				i4Ret = SHM_OPT_FAIL;					
				break;
			}
			--nLoop;
			SHM_COMM_LOG(SHM_LOG_DEBUG, "try write again!");
...
int  shm_read(chn_comm_ctlinfo * pCCInfo, void * pChr, uint32_t binSize, int streamDirect) {
	...
	int i4Ret = SHM_OPT_SUCC;
	if (NULL == pCCInfo || NULL == pChr || (0 > streamDirect || streamDirect >= MAX_CHN_NUM)) {
		SHM_COMM_LOG(SHM_LOG_ERROR, "NULL == pCCInfo || NULL == pChr");
		return SHM_OPT_FAIL;
	}
	...
	if (SHM_OPT_FAIL == checkMQHead(&(pCCInfo->chn_list[streamDirect]))) {
		...
		while (rLen != binSize)
		{
			if (nLoop < 0)
			{
				SHM_COMM_LOG(SHM_LOG_ERROR, "try out, rLen != binSize");
				i4Ret = SHM_OPT_FAIL;					
				break;
			}
			--nLoop;
			SHM_COMM_LOG(SHM_LOG_DEBUG, "try read again!");
			usleep(TIME_SLEEP_RW);
...
int  fini_memqueue(int mqSize, chn_comm_ctlinfo * pCCInfo, int streamDirect) {
	...
	if (NULL == pCCInfo || (0 > streamDirect || streamDirect >= MAX_CHN_NUM)) {
		SHM_COMM_LOG(SHM_LOG_ERROR, "NULL == pCCInfo or streamDirect [%d] illegal", streamDirect);
		return SHM_OPT_FAIL;
	}
    ...
    SHM_COMM_LOG(SHM_LOG_INFO, "fini_memqueue succ");
	return i4Ret;
...
int  fini_shmfile(int mqSize, chn_comm_ctlinfo * pCCInfo, int streamDirect) {
	...
	if ((NULL != pCCInfo->chn_list[streamDirect].head_maped)
		...
		SHM_COMM_LOG(SHM_LOG_ERROR,"error munmap SHM_FILE: %d", errno);
		perror("munmap");
		return SHM_OPT_FAIL;
	}
...
int remove_shmfile(const char * pShmName) {
	...
	if (NULL == pShmName) {
		SHM_COMM_LOG(SHM_LOG_ERROR, "NULL == pShmName");
		return SHM_OPT_FAIL;
	}
	...
	if (0 == access(pShmName, F_OK)) {
		int ret = remove(pShmName);
		if (0 != ret) {
			SHM_COMM_LOG(SHM_LOG_ERROR,	"remove %s fail, errno:%d", pShmName, errno);
			i4Ret = SHM_OPT_FAIL;
		}
	} else {
		SHM_COMM_LOG(SHM_LOG_ERROR, "%s not exist!", pShmName);
		i4Ret = SHM_OPT_SUCC;
	}
